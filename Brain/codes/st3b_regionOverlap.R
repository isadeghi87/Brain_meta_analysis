...
gp = ggplot(data = allExpr, 
  ...
  geom_density_ridges()+
  geom_vline(xintercept = 0, lty = 2 , color = "red", size = 0.2)+
  facet_wrap(~Region, nrow = 1)+
  theme_ridges(center_axis_labels = T)+
  theme(text = element_text(size = 12, colour = "black"),
        legend.position = "none")
gp = gp + labs(y = "Disease", x = "logFC")+
...
for (dx in unique(allExpr$Disease)){
  for(region in unique(allExpr$Region)){
    ...
    dat = subset(dat , p.value < 0.05)#significant
...
#ggtable
DEG.table = ggtexttable(DEGs,theme = ttheme("mBlue"))
saveRDS(DEG.table,"./codes/fig3b_DEGs.rds")
ggsave(plot = DEG.table, file = "./results/figures/Regions_DEGs.pdf")

#volcano plot
library(ggrepel)
pointCol = ifelse(-log10(allExpr$p.value)>1.3 & abs(allExpr$logFC)> 0.5, "black", "grey")
g =ggplot(data = allExpr,
       aes(x = logFC, y = -log10(p.value)))+
  geom_point(color = pointCol , size = 0.7)+
  geom_label_repel(data = subdat,
                   aes(label = symbol),
                   color = ifelse(subdat$logFC >0, "green", "indianred3"),
                   size = 2)+
...
#remove frequency <1
freq = subset(freq, Frequency >1)

###
id= as.character(freq$gene)

top_overlap =subdat[subdat$symbol %in% id,]
ggplot(top_overlap, aes(x= logFC, y = symbol, color= symbol,size = -log10(FDR)))+
  geom_point()+
