...
g.copy <- delete.edges(g1, which(E(g1)$weight <0.6))
...
plot.igraph(g.copy, 
            ...
            edge.color="grey")
title(main = "Top 20 hub genes per module", cex.main = 1)
...
pdf(file = "./results/figures/CellTypesHubgenes.pdf",
    height = 15,
    width = 18)
plot.igraph(g.copy,
            vertex.label = geneSymbols,
            ...
            layout=layoutfr,
            edge.color="grey",
            mark.groups = cellGroups,
            mark.shape = 1/2,
             mark.col = NA,
             mark.border = 1:8,
            main = "Top hub genes of cell-type specific modules ")
