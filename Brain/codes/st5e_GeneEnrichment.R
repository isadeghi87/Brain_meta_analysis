...
library(pSI); library(gProfileR); library(ggplot2); library(biomaRt); library(WGCNA)
library(ggpubr); library(pheatmap); library(dplyr); library(magrittr)
library(GeneOverlap); library(factoextra);library(FactoMineR)

setwd("../Desktop/Iman/Brain_meta_analysis_project-20191130T221845Z-001/Brain_meta_analysis_project/codes/")
#---load final network
...
ca_plot
ggsave(filename = "../Main_manuscript/Figures/Corresp.Analysis.pdf", 
       plot = ca_plot, width = 9, height = 7)
...
pheatmap(-log10(cell_mod),
         color = redWhiteGreen(2000,1)[1050:2000],
       #  breaks = ,
         cluster_cols = T,
         cluster_rows = T,
       #   fontsize_col = 15,
...
pdf("./results/figures/TFBsPlot.pdf",width = 6, height=4)
par(oma=c(0,5,0,3), mar=c(5,7,1,0))
bp = barplot(-log10(topTF$p.value), 
             main="Top TF binding sites for modules",
             horiz=T, yaxt='n', 
             col=as.character(topTF$m),
             xlab='-log10(FDR)',cex.main=1, cex.axis = .7, border=NA)
axis(2,at=bp,labels=labels,tick=FALSE,las=2,cex.axis=0.6);
abline(v=-log10(0.01), col="black", lwd=2,lty=2)
dev.off()
...
par(margin(2,2,2,2))
# png("./results/figures/BrainEnhancerEnrichment.png", width = 20, height = 15, res = 300, units = "cm")
heatmap.2(-log10(to_plot),
           col=redWhiteGreen(2000,1)[1060:2000],
           # col= brewer.pal(11, name= "Greens"),
          scale="none",
          trace="none",
          cexRow = 1.5,
          cexCol = 1.5, 
          ...
          notecex=1.5, 
          notecol="black",
          RowSideColors = rownames(to_plot),
          main="Enrichment of Brain Enhancers")
dev.off()

pheatmap(-log10(to_plot),
         color = bluered(n = 2000)[1000:0],
         cluster_cols = F,
         cluster_rows = T,
         fontsize_number = 6,
         fontsize_row = 10,
         treeheight_row = 10,fontsize_col = 10,
         cellwidth = 20,
         cellheight = 20,
         angle_col = 45,
         show_rownames = T,
...
brain_cells_adjusted = adjustBrainCells(exp,
                                        nMarker = 100, species = "combined/mega_analysis")
