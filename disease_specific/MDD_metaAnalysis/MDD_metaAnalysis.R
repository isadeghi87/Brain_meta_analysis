#--- meta analysis for MDD using batch correction and linear model
setwd("C:/Users/crgcomu/Desktop/Iman/Brain_meta/analysis_rawCount/disease_specific/MDD_metaAnalysis/")
...
MDD_datMeta = multiExpr[["MDD"]]$datMeta 
MDD_datExpr <- as.matrix(multiExpr[["MDD"]]$datExpr)
rm(multiExpr,allExp,allMeta)

MDD_datMeta$Dx = factor(MDD_datMeta$Dx)
MDD_datMeta$Study = factor(MDD_datMeta$Study)
MDD_datMeta$Brain_Region = factor(MDD_datMeta$Brain_Region)
MDD_datMeta$Brain_Lobe = factor(MDD_datMeta$Brain_Lobe)

dx = as.numeric(as.factor(MDD_datMeta$Dx))
st = as.numeric(as.factor(MDD_datMeta$Study))
rg = as.numeric(as.factor(MDD_datMeta$Brain_Region))

# log cpm 
lcpm = cpm(MDD_datExpr,log = T)

# plot
pdf("./figures/pca/pca_no_norm.pdf", width = 6, height = 10)
par(mfrow=c(2,1))
plotMDS(log2(lcpm),col= st, pch = dx, main = "MDD-study")
plotMDS(log2(lcpm),col= rg, pch = dx, main = "MDD brain region")
dev.off()


#### filter genes ####
id = rowSums(cpm(MDD_datExpr)>0.5)>= 0.3 *ncol(MDD_datExpr)
datExp  = MDD_datExpr[id,]
design = model.matrix(~Dx, data = MDD_datMeta)
v = voom(datExp,design =design)

#### batch correction
datExp.comb = ComBat(dat = as.matrix(v$E),
                     batch = MDD_datMeta$Study,
                     mod = design)
...
for(t in thet){
  for ( p in perp){
    for (i in int){
      ...
      dat$Disease = MDD_datMeta$Dx
      dat$Region = MDD_datMeta$Brain_Lobe
      dat$Study = MDD_datMeta$Study
...
design = model.matrix(~Dx + Sex + Brain_Lobe, data = MDD_datMeta)
...
for(i in 1:nrow(datExp.comb)) {
  ...
  tryCatch({
    meta[i,] = summary(lme(expr~ Dx  + Sex + Brain_Lobe ,data = MDD_datMeta, random=~1|Subject_ID))$tTable[2,c(1,2,5)]
  }, error=function(e){})
}
...
#save data
write.csv(file = "./tables/MDD_sumstats.csv",x = meta)

#### DGE for brain lobe ####

lobe_stats <- vector("list", length = length(unique(MDD_datMeta$Brain_Lobe)))
names(lobe_stats) <- levels(MDD_datMeta$Brain_Lobe)
for (lobe in levels(MDD_datMeta$Brain_Lobe)){
  idx = which(MDD_datMeta$Brain_Lobe == lobe)
  Meta = MDD_datMeta[idx,]
...
write.csv(lobe_sumstat, "./tables/MDD_lobe_sumstats.csv")

save(file = "./MDD_metaAnalysis.Rdata",MDD_datExpr,MDD_datMeta,datExp,datExp.comb)
