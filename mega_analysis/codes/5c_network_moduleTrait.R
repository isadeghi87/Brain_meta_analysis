...
mod_col = levels(bpdata$color)
names(mod_col) = mods[match(mod_col,names(mods))]
...
byModule = ggplot(bpdata,
  ...
  geom_errorbar(aes( ymin = (beta - SEM),
                ...
                size = 0.3,
                width =.3) +
  scale_fill_manual(name="Diagnosis", values = mod_col) + 
  labs(y="beta", 
       x="") + 
  geom_text(color = "red",
            size = 4, 
...
byDisease = ggplot(bpdata, 
  ...
  #theme_minimal() + 
  scale_fill_manual(name = "Dx", values = mod_col) +
  labs(y = "beta", 
       x = "",
       title = "") +
  geom_text(
